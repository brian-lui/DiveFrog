...
function Fighter:init2(init_player, init_foe, init_super, init_dizzy, init_score)
	...
	self.sprite = love.graphics.newQuad(
		self.image_index * self.sprite_size[1],
		0,
		self.sprite_size[1],
		self.sprite_size[2],
		self.image_size[1],
...
function Fighter:jump_key_press()
	...
	for bufferframe = 0, 2 do
		if self.player == 1 then
			...
			if p1_frame_attack and not p1_prev_frame_attack then
				both_keys_down = true
			end
		elseif self.player == 2 then
			...
			if p2_frame_attack and not p2_prev_frame_attack then
				both_keys_down = true
			end
		end
	end
...
function Fighter:gotKOed() -- keep calling this until self.isKO is false
	if frame - round_end_frame < 60 then
		self.vel = {0, 0}
		self.gravity = 0
		self.life = math.max(self.life - 6, 0)
...
local function placeBubble(h_pos, v_pos, h_move, v_move, delay)
...
function Fighter:hitOpponent() -- execute this one time, when you hit the opponent
	...
	else -- otherwise single speechbubble
		-- buggy with hotflame for now
...
function Fighter:victoryPose() -- keep calling this if self.hasWon is true
	...
	if frame - round_end_frame == 60 then
		if not music.currentBGM2:isPlaying() then
...
function Fighter:getCenter()
	return self.pos[1] + 0.5 * self.sprite_size[1]
end

function Fighter:addScore()
		self.score = self.score + 1
end

function Fighter:setFrozen(frames)
	self.frozenFrames = frames
end

function Fighter:updateImage(image_index)
	self.sprite = love.graphics.newQuad(
		image_index * self.sprite_size[1],
		0,
		self.sprite_size[1],
		self.sprite_size[2],
		self.image_size[1],
...
function Fighter:updatePos()
	...
	if self.frozenFrames == 0 then
		...
		if self.isInAir then
			self.vel[2] = self.vel[2] + (self.gravity * self.vel_multiple * game.speed)
		end
...
function Fighter:updateSuper()
	...
	if self.isSupering and not (self.isKO or self.hasWon) then
		...
		-- after-images
		local shadow = particles.AfterImage(
			self.image,
			self.image_size,
			self.sprite_size,
			1
