...
function Jean:extraStuff()
	-- custom friction
	if self.dandy or self.pilebunking then
		self.vel[1] = self.vel[1] / 1.3
	end

	-- during dandy step's slowing end part, allow pilebunker
	if self.dandy and
	math.abs(self.vel[1]) >= (0.1 * game.speed) and
	math.abs(self.vel[1]) < (3.6 * game.speed) then
	...
	if self.pilebunking and
	math.abs(self.vel[1]) >= (0.001 * game.speed) and
	math.abs(self.vel[1]) < (1.2 * game.speed) then
