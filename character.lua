...
function Fighter:initialize(init_facing)
  ...
  self.image_size = {2, 2}
  self.image_index_h = 0 -- Horizontal offset starting at 0
  ...
  self.hurtboxes_standing_sides = {{0, 0, 0, 0}}
  self.hurtboxes_jumping_sides  = {{0, 0, 0, 0}}
  self.hurtboxes_falling_sides = {{0, 0, 0, 0}}
  self.hurtboxes_attacking_sides  = {{0, 0, 0, 0}}
  self.hurtboxes_kickback_sides  = {{0, 0, 0, 0}}
  self.hurtboxes_ko_sides  = {{0, 0, 0, 0}}
  self.hitboxes_attacking_sides = {{0, 0, 0, 0}}
  ...
  self.sprite = love.graphics.newQuad(self.image_index_h * self.sprite_size[1], 0, self.sprite_size[1], self.sprite_size[2], self.image_size[1], self.image_size[2])
  self.facing = init_facing -- 1 for facing right, -1 for facing left
  if init_facing == 1 then self.image_index_v = 0 else self.image_index_v = 1 end
  ...
  self.current_hurtboxes = self.hurtboxes_standing_sides
  self.current_hitboxes = self.hitboxes_attacking_sides
end

  function Fighter:jump_key_press()
    -- check special move
    local attack_already_down = false -- check if attack key was down on (frame - 3)
    ...
    for bufferframe = 0, 2 do -- check for attack key down from (frames - 2) to frame
      ...
      if self.player == 1 then -- check for attack key down at (frame - 3)
        if keybuffer[(frame - 3)][2] then attack_already_down = true end
      elseif self.player == -1 then
        if keybuffer[(frame - 3)][4] then attack_already_down = true end
      end
    end

    if attack_within2_down and not attack_already_down and self.in_air then
      self:air_special()
    elseif attack_within2_down and not attack_already_down and not self.in_air then
      self:ground_special()
    end
  ...
  function Fighter:attack_key_press()
    -- check special move
    local jump_already_down = false -- check if jump key was down on (frame - 3)
    ...
    for bufferframe = 0, 2 do -- check for jump key down from (frames - 2) to frame
      ...
      if self.player == 1 then -- check for jump key down at (frame - 3)
        if keybuffer[(frame - 3)][1] then jump_already_down = true end
      elseif self.player == -1 then
        if keybuffer[(frame - 3)][3] then jump_already_down = true end
      end
    end

    if jump_within2_down and not jump_already_down and self.in_air then
      self:air_special()
    elseif jump_within2_down and not jump_already_down and not self.in_air then
      self:ground_special()
    end
  ...
  function Fighter:jump(h_vel, v_vel)
    ...
    self:updateImage(1, self.image_index_v)
    self.current_hurtboxes = self.hurtboxes_jumping_sides
  end
  ...
  function Fighter:kickback(h_vel, v_vel)
    ...
    self.gravity = self.default_gravity
    self.vel = {h_vel * self.facing, -v_vel}
    self:updateImage(3, self.image_index_v)
    self.current_hurtboxes = self.hurtboxes_kickback_sides
  end
  ...
  function Fighter:land() -- called when character lands on floor
    ...
    if not self.ko then
      self.vel = {0, 0}
      self:updateImage(0, self.image_index_v)
      self.current_hurtboxes = self.hurtboxes_standing_sides
    end
  end
  ...
  function Fighter:attack(h_vel, v_vel)
    self.vel = {h_vel * self.facing, v_vel}
    self.attacking = true
    self:updateImage(4, self.image_index_v)
    self.gravity = 0
    self.current_hurtboxes = self.hurtboxes_attacking_sides
...
function Fighter:gotHit() -- execute this one time, when character gets hit
  self.vel_multiple = 1.0
  self.ko = true 
...
function Fighter:koRoutine() -- keep calling koRoutine() until self.ko is false
  ...
  if frame - round_end_frame > 60 then
    ...
    self:updateImage(5, self.image_index_v)
    self.current_hurtboxes = self.hurtboxes_ko_sides
  end
end
...
function Fighter:wonRoundRoutine() -- keep calling this if self.won is true
  ...
  if frame - round_end_frame > 60 then
    ...
    self.attacking = false
    if self.in_air then
      self:updateImage(2, self.image_index_v)
    else
      self:updateImage(0, self.image_index_v)
    end
    self.current_hurtboxes = self.hurtboxes_standing_sides 
...
function Fighter:getSprite_Size() return unpack(self.sprite_size) end
...
function Fighter:setNewRound()
  ...
  self.vel_multiple = 1.0
  self.gravity = self.default_gravity
  if self.facing == 1 then self.image_index_v = 0 else self.image_index_v = 1 end
  self.opp_center = 400 -- center of opponent's sprite
  self.my_center = self.pos[1] + self.sprite_size[1]
  self:updateImage(self.image_index_h, self.image_index_v)
  self.current_hurtboxes = self.hurtboxes_standing_sides
  self.current_hitboxes = self.hitboxes_attacking_sides
end

function Fighter:updateImage(image_index_h, image_index_v)
  self.sprite = love.graphics.newQuad(image_index_h * self.sprite_size[1], image_index_v * self.sprite_size[2], self.sprite_size[1], self.sprite_size[2], self.image_size[1], self.image_size[2])
  return self.sprite
end
...
function Konrad:initialize(init_facing)
  ...
  --lists of hitboxes and hurtboxes for the relevant sprites. format is LEFT, TOP, RIGHT, BOTTOM, relative to top left corner of sprite.
  self.hurtboxes_standing_sides = {{78, 33, 116, 50}, {83, 16, 114, 33}, {69, 51, 127, 79}, {76, 85, 120, 101}, {70, 105, 129, 142}, {75, 143, 124, 172}, {82, 173, 120, 190}}
  self.hurtboxes_jumping_sides  = {{78, 33, 116, 50}, {83, 16, 114, 33}, {69, 51, 127, 79}, {76, 85, 120, 101}, {70, 105, 129, 142}, {75, 143, 124, 172}, {82, 173, 120, 190}}
  self.hurtboxes_falling_sides = {{78, 33, 116, 50}, {83, 16, 114, 33}, {69, 51, 127, 79}, {76, 85, 120, 101}, {70, 105, 129, 142}, {75, 143, 124, 172}, {82, 173, 120, 190}}
  self.hurtboxes_attacking_sides  = {{66, 9, 96, 34}, {67, 35, 108, 54}, {75, 55, 104, 103}, {68, 104, 91, 135}, {100, 105, 114, 136}, {111, 137, 128, 157}, {125, 158, 138, 183}}
  self.hurtboxes_kickback_sides  = {{83, 14, 119, 40}, {67, 41, 128, 64}, {70, 65, 110, 165}, {72, 166, 111, 182}}
  self.hurtboxes_ko_sides  = {{0, 0, 0, 0}}
  self.hitboxes_attacking_sides = {{119, 166, 135, 183}}
...
function Jean:initialize(init_facing)
  ...
  --lists of hitboxes and hurtboxes for the relevant sprites. format is LEFT, TOP, RIGHT, BOTTOM, relative to top left corner of sprite.
  self.hurtboxes_standing_sides = {{53, 33, 95, 50}, {58, 16, 89, 33}, {44, 51, 102, 79}, {51, 85, 95, 101}, {45, 105, 104, 142}, {50, 143, 99, 172}, {57, 173, 95, 190}}
  self.hurtboxes_jumping_sides  = {{53, 33, 95, 50}, {58, 16, 89, 33}, {44, 51, 102, 79}, {51, 85, 95, 101}, {45, 105, 104, 142}, {50, 143, 99, 172}, {57, 173, 95, 190}}
  self.hurtboxes_falling_sides = {{53, 33, 95, 50}, {58, 16, 89, 33}, {44, 51, 102, 79}, {51, 85, 95, 101}, {45, 105, 104, 142}, {50, 143, 99, 172}, {57, 173, 95, 190}}
  self.hurtboxes_attacking_sides  = {{7, 9, 35, 23}, {10, 26, 58, 69}, {61, 58, 77, 69}, {31, 72, 83, 109}, {42, 110, 93, 126}, {61, 129, 116, 149}, {118, 138, 131, 149}, {62, 151, 145, 172}}
  self.hurtboxes_dandy_sides  = {{11, 4, 37, 76}, {15, 82, 45, 129}, {24, 131, 61, 151}, {62, 142, 73, 151}, {33, 152, 82, 166}, {47, 167, 95, 186}}
  self.hurtboxes_ko_sides  = {{0, 0, 0, 0,}}
  self.hurtboxes_pilebunker_sides = {{6, 32, 37, 71}, {17, 68, 71, 137}, {73, 128, 100, 137}, {42, 140, 108, 187}, {110, 152, 118, 187}}
  self.hurtboxes_pilebunkerB_sides = {{15, 32, 46, 71}, {17, 68, 71, 137}, {73, 128, 83, 137}, {42, 140, 98, 187}, {100, 165, 113, 180}}

  self.hitboxes_attacking_sides = {{130, 154, 147, 172}}
  self.hitboxes_pilebunker_sides = {{86, 85, 148, 92}}
  ...
  function Jean:dandyStep(h_vel)
    ...
    self:updateImage(3, self.image_index_v)
    self.current_hurtboxes = self.hurtboxes_dandy_sides
    self.friction_on = false
    if self.super < 96 and not self.super_on then 
  ...
  function Jean:pilebunk(h_vel)
    ...
    self.vel[1] = h_vel * self.facing
    self:updateImage(6, self.image_index_v)
    self.current_hurtboxes = self.hurtboxes_pilebunker_sides
    self.current_hitboxes = self.hitboxes_pilebunker_sides
    if self.super < 96 and not self.super_on then 
  ...
  function Jean:extraStuff()
    ...
    if self.pilebunking and math.abs(self.vel[1]) >= 0.001 and math.abs(self.vel[1]) < 1 then
      self.attacking = false
      self:updateImage(7, self.image_index_v)
      self.current_hurtboxes = self.hurtboxes_pilebunkerB_sides
    end
